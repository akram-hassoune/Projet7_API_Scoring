...
def get_client_ids():
    response = requests.get("http://localhost:8000/client_ids/")
...
def get_client_info(client_id):
    ...
    response_client_info = requests.get(f"http://localhost:8000/client_info/{client_id}")
    client_info = response_client_info.json()["client_info"]

    # Appeler l'API pour obtenir les informations de prêt
    response_pret_info = requests.get(f"http://localhost:8000/client_pret/{client_id}")
...
def get_default_prediction(client_id):
    ...
    response_default_prediction = requests.get(f"http://localhost:8000/client_scores/{client_id}")
